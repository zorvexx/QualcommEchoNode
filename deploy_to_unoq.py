...
KNOWN_IPS = ["10.51.210.105", "192.168.29.219", "unoq.local", "arduino.local"]
...
def deploy(mode="MONITORING", machine_id="laptop_01", session_id="idle_01"):
    ...
    local_models_dir = os.path.join(workspace, "ml_pipeline", "models")
    if not os.path.exists(local_models_dir):
        local_models_dir = os.path.join(workspace, "models")
        
    local_main_py = os.path.join(workspace, "ml_pipeline", "edge_main.py")
    if not os.path.exists(local_main_py):
        local_main_py = os.path.join(workspace, "edge_main.py")
        
    board_ip = find_board_ip()
    print("=" * 65)
    print(f"  DEPLOYING RETROFIT TO ARDUINO UNO Q @ {board_ip}")
    print(f"  TARGET MODE: [{mode.upper()}] | MACHINE: [{machine_id}]")
    print("=" * 65)
    ...
    try:
        ...
        if os.path.exists(local_main_py):
            print(" -> Uploading latest dual-mode engine to Uno Q...")
        ...
        if mode.upper() == "MONITORING":
            ...
            print("  - Data Acquisition is ACTIVE")
            print("  - Ready to record new healthy machine datasets")
        print("=" * 65)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mode", default="MONITORING", choices=["COLLECTION", "MONITORING", "collection", "monitoring"])
